...
1. Project structure needed?
   → serena: get_symbols_overview("{root or affected directory}") — ONE call.
...
3. External pattern or comparison needed?
   → fetch("{url}") if you have a direct URL
   → ELSE duckduckgo-mcp: one query ≤7 words. Stop after first useful result.

4. GitHub issue or RFC context needed?
   → github: get_issue — base decisions on actual requirements.

5. Decision has >4 trade-offs or multi-hop reasoning?
...
□ serena: get_symbols_overview("{most relevant directory}")
...
1. **Decision summary** — ≤5 bullets, each actionable
