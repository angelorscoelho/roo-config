...
- Taint path more than 4 serena calls: report partial chain, ask to continue.
...
Phase 1: MAP ATTACK SURFACE (no tools yet)
  ...
  If surface more than 3 categories: crash-mcp scaffold first.

Phase 2: TAINT ANALYSIS
  crash-mcp: scaffold multi-hop chain FIRST.
  serena: find_symbol for input handler — entry point.
  serena: find_referencing_symbols — trace input flow.
...
Phase 3: SECRETS and MISCONFIGS
  ...
  serena: get_symbols_overview for infra/ — public buckets, open ports, missing HTTPS.
  serena: get_symbols_overview for .github/workflows/ — secret exposure in CI.
...
Phase 5: AUTH and AUTHZ FLOWS
  ...
  serena: find_referencing_symbols to ensure all protected routes use it.
