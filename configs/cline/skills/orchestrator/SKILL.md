...
Step 1: LOAD CONTEXT FIRST
  ...
  Task requires understanding project scope?
    serena: get_symbols_overview for affected directories — ONE call.
  ...
  Task references a spec/RFC URL?
    fetch — include key constraints in delegation prompts.

Step 2: SCAFFOLD IF COMPLEX (more than 5 sub-components or unclear dependency order)
    crash-mcp: purpose=planning BEFORE writing the todo list.

Step 3: BUILD TODO LIST
  One item per atomic task.
  Format: skill: deliverable (input to output)

Step 4: WRITE DELEGATION PROMPTS (200 tokens max each)
...
2. Delegation assignments (one per sub-task, 200 tokens max each)
3. Success criteria
